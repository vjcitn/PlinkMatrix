...
# Subsetting with GenomicRanges

A `RangedPlink` class has been defined with a `subsetByOverlaps` method
to simplify iteration over tiled genomes.

```{r lkra}
data(example_GRanges)
ran1 = RangedPlink(gdemo, example_GRanges)
little = GenomicRanges::GRanges("chr18:1-100000")
subsetByOverlaps(ran1, little)
```

No subsetting by samples is permitted -- just build a new
RangedPlink instance to accomplish this.
